...
exports.apihehe = function(req, res){
  // res.send(req.params);

  global.nrptemp = req.params.username;
  global.gatetemp = req.params.grup;

  User.findOne({ nrp: req.params.username }) 
  .then((doc) => {
     if (doc) {
       res.send("nrp sudah tersedia")
     } else {
       console.log("no data exist for this id");
       let newuser = new User({nrp:req.params.username, password:req.params.password, role:"1"}); // this is modal object.
       newuser.save()
         .then((data)=> {

          Usergate.findOne({idgate:req.params.grup, nrp:req.params.username}) 
            .then((doc) => {
              if (doc) {
                 res.send("if usergate")
                // res.redirect('/admin');
              } else {
                console.log("no data exist for this id");
                let newusergate = new Usergate({idgate:req.params.grup, nrp:req.params.username}); // this is modal object.
                newusergate.save()
                  .then((data)=> {
                     res.send("berhasil");
                    // res.redirect('/admin');
